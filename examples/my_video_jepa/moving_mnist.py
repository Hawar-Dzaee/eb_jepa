...
class MovingMNISTDet(MovingMNIST):
    def __init__(self, transform=None, split=None, map_size=8): 
        ...
        Args:
            ...
            split (str): train or val 
            map_size (int): size of map to predict position over 

        Returns: 
            video: [C, T, H, W] - greyscale video frames 
            digit_location: [T, map_size, map_size] - Coarse binary heatmap for digit
        """
        ...
        for idx, frames in enumerate(self.data): 
            for t in range(T):  # why not `for frame in frames:`
    ...
    def __getitem__(self,idx):
        ...
        digit_locations = self.digit_locations[idx]
